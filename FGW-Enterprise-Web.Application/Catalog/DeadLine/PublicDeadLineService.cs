    ...
    public class PublicDeadLineService : IPublicDeadLineService
    {
        ...
        public async Task<PagedResult<DeadlineViewModel>> GetAllByCategoryId(GetDeadlinePagingRequest request)
        {
            var query = from d in _context.Deadline
                        ...
                        join rdc in _context.DeadlineCate on dr.rd_DeadineCate equals rdc.dlCate_Id
                        join du in _context.UserFile on d.dl_Id equals du.file_DeadlineId
                        select new { d};
            //2.filter
            if (request.deadlineCateId != null)
            {
                query = query.Where(x => x.d.dl_Id==request.deadlineCateId);
            }
